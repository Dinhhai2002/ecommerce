...
- Follow the existing package structure:
  ...
  ```
- Package structure for dao and service follows a specific pattern:
  - Dao interfaces must be placed directly in `dao` package (e.g., `ProductDao.java`)
  - Dao implementations must be placed in `dao.impl` package (e.g., `ProductDaoImpl.java`)
  - Service interfaces must be placed directly in `service` package (e.g., `IProductService.java`)
  - Service implementations must be placed in `service.impl` package (e.g., `ProductService.java`)
...
## Response Standards
- All API responses must use the standardized response format provided by BaseController
- Use the `buildResponse` method from BaseController to ensure consistent response structure
- The standard response format includes:
  - `data`: The actual response data
  - `status`: HTTP status code
  - `messageError`: Error message (if any)
- Custom endpoints added to controllers extending BaseController must also use this response format
- Example: 
  ```java
...
- **Entity Layer**: 
  - Fields with compound names should use camelCase in Java but snake_case in database
  - Example: `parentId` in Java maps to `parent_id` in database
  - Use `@Column(name = "column_name")` to specify the database column name
  - Example: `@Column(name = "user_id") private int userId;`

- **Response Layer**:
  - Fields with compound names should use camelCase in Java but snake_case in JSON
  - Use `@JsonProperty("field_name")` to specify the JSON field name
  - Example: `@JsonProperty("parent_id") private int parentId;`
...
- [ ] Field naming conventions are followed (camelCase in Java, snake_case in DB/JSON)
- [ ] All API responses use the standardized response format from BaseController
