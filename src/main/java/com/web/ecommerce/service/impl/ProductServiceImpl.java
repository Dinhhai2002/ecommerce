...
public class ProductServiceImpl extends BaseServiceImpl<Product, Integer> implements ProductService {
    ...
    public List<ProductOptionsResponse> findProductOptionsWithDetailsById(Integer productId) {
        ...
        List<Integer> optionIds = options.stream()
                .map(ProductOptions::getOptionId)
                .filter(id -> id != null)
                .collect(Collectors.toList());
                
        List<Integer> optionValueIds = options.stream()
                .map(ProductOptions::getOptionValueId)
                .filter(id -> id != null)
                .collect(Collectors.toList());
        ...
        List<OptionValues> allOptionValues = optionValueIds.isEmpty() ? new ArrayList<>() : optionValuesService.findByIds(optionValueIds);
    ...
    public List<CampaignResponse> findProductCampaignsById(Integer productId) {
        ...
        if (!campaignProducts.isEmpty()) {
            ...
            List<Integer> campaignIds = campaignProducts.stream()
                    .map(CampaignProduct::getCampaignId)
                    .collect(Collectors.toList());
        ...
        if (campaignResponses.isEmpty() && product.getCategoryId() != null) {
            List<CampaignCategory> campaignCategories = campaignCategoryService.findByCategoryId(product.getCategoryId());
            
            if (!campaignCategories.isEmpty()) {
                // Extract campaign IDs
                List<Integer> campaignIds = campaignCategories.stream()
                        .map(CampaignCategory::getCampaignId)
                        .collect(Collectors.toList());
